...
/*
    ...
    线程上下文类加载器的重要性：(改变了双亲委托模型)


    SPI(Service Provider Interface)

    父ClassLoader可以使用当前线程Thread.currentThread().getContextClassLoader()所指定的classloader所加载的类。
    这就改变了父ClasLoader不能使用子ClassLoader或者其他没有直接父子关系的ClassLoader所加载的类的情况，即改变了
    双亲委托模型。
    ...
    来源的jar包的，这样传统的双亲委托模型就无法满足SPI的要求。而通过给线程设置上下文类加载器，就可以由设置的上下文类加载器
    来实现对于接口实现类的加载。
