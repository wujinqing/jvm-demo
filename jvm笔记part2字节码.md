...
>  3.基于栈的指令集的优势在于它可以在不同平台之间移植，而基于寄存器的指令集是与硬件架构紧密相关的，无法做到可移植。

>  4.基于栈的指令集的缺点在于完成相同的操作，指令数量要比基于寄存器的指令集数量要多；基于栈的指令集是在内存中完成操作的，
>  而基于寄存器的指令是直接由CPU来执行的，它是在高速缓冲区中执行的，速度要快很多。虽然Java虚拟机可以采用一些优化手段，但总体来说，基于栈的指令集的执行速度要慢一些。
